    ...
    mistakes: @js($mistakes)
        ...
        <section class="py-10">
            <div class="container">
                    ...
                    <div x-show="activeTab === '{{ $category->slug }}'" class="flex flex-wrap justify-center gap-6 lg:gap-4">
                        @foreach($category->tests as $test)
                            <div class="flex flex-col gap-4 p-5 w-full md:max-w-[280px] items-center text-center bg-gray-200 rounded-xl shadow-md">
                                ...
                                <p class="text-sm text-gray-600">{{ $test->duration }} минут</p>
                ...
                <!-- Сохранённые вопросы -->
                {{--                    <div x-show="activeTab === 'saved'" class="flex flex-col items-center gap-6">--}}
                {{--                        <h2 class="text-xl font-semibold text-center w-full">Сохранённые вопросы</h2>--}}

                {{--                        @if($savedQuestions->isEmpty())--}}
                {{--                            <p class="text-gray-500">У вас пока нет сохранённых вопросов.</p>--}}
                {{--                        @else--}}
                {{--                            <a href="{{ route('questions.saved') }}"--}}
                {{--                               class="px-5 py-3 bg-orange-400 text-white font-semibold rounded-lg hover:bg-white hover:text-black border border-orange-400">--}}
                {{--                                Пройти сохранённые вопросы--}}
                {{--                            </a>--}}
                {{--                        @endif--}}
                {{--                    </div>--}}
                ...
                <!-- Errors section -->
                    <div x-show="activeTab === 'errors'" class="flex flex-col items-center gap-6 w-full">
                        <h2 class="text-xl font-semibold text-center w-full">Мои ошибки</h2>

                        <template x-if="mistakes.length === 0">
                            <p class="text-gray-500">Ошибок пока нет.</p>
                        </template>
                        ...
                        <template x-for="mistake in mistakes" :key="mistake.id">
                            <div class="space-y-4 w-full lg:w-[800px] mb-5 border rounded-lg shadow-md bg-gray-50 p-4">
                                <h3 class="text-lg font-medium text-gray-900" x-text="mistake.question.question"></h3>
                                ...
                                <div class="flex flex-col gap-2">
                                    <p class="font-semibold">Пояснение:</p>
                                    <p class="leading-relaxed text-gray-700" x-text="mistake.question.explanation"></p>
                    ...
                    <h2 class="text-xl font-semibold text-center mb-6">Результаты тестов</h2>
                    ...
                    <div class="flex flex-wrap justify-center gap-6">
                        @foreach($testResults as $result)
                            ...
                            <div class="flex flex-col items-center p-5 w-full md:max-w-[280px] text-center rounded-xl shadow-md {{ $isSuccess ? 'bg-green-100' : 'bg-red-100' }}">
                                ...
                                <p class="text-sm mt-2">Результат: <strong>{{ $result->score }}/{{ $result->total }}</strong></p>
